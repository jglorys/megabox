...
<div class="d-flex justify-content-center">
	...
	<c:forEach var="reservationSchedule" items="${reservationScheduleList}">
		<div class="p-4 m-4" style="background-color: #E6E6E6; border-radius:2em;">
			<span class="font-weight-bold p-1">${reservationSchedule.schedule.movieTitle}</span><br><br>
			<span class="font-weight-bold p-1">${reservationSchedule.schedule.auditoriumName}관</span><br><br>
			<span class="font-weight-bold p-1">${fn:substring(reservationSchedule.schedule.time,0,4)}.${fn:substring(reservationSchedule.schedule.time,4,6)}.${fn:substring(reservationSchedule.schedule.time,6,8)}  |   
			${fn:substring(reservationSchedule.schedule.time,8,10)}시 ${fn:substring(reservationSchedule.schedule.time,10,12)}분
			</span><br><br>
			<span class="font-weight-bold p-1">성인 ${reservationSchedule.reservation.adult}명 / 청소년 ${reservationSchedule.reservation.adolescent}명 / 경로 ${reservation.senior}명</span><br><br>
			<span class="font-weight-bold p-1">좌석 : ${fn:replace(reservationSchedule.reservation.seatLocation, ',', ' & ')}</span><br><br>
			<span class="font-weight-bold p-1">결제금액 : ${reservationSchedule.reservation.payment}원</span><br><br>
